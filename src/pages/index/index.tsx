...
import { GetLessonCategoryRequest } from '../../common/Server'
// import { LessonModel} from '../../common/HomeInterfaces'

import { HomeCardView, HomePage} from '../../components'
import { AtActivityIndicator, AtMessage, AtTabs, AtTabsPane } from 'taro-ui'
...
export default class Index extends Component<MyProps, MyState> {
  ...
  componentWillMount() {
    GetLessonCategoryRequest().then(res=>{
      console.log(res)
  ...
  render () {
    // let tabs = this.state.titles.map((item) => {
    //     // {title: item }
    //     new Item(item)
    ...
    this.state.titles.map((item) => {
      tabs.push(new Item(item))
    })
    return (
      <AtTabs
        current = {this.state.current}
        scroll = {tabs.length > 4 ? true : false}
        tabList = {tabs}
        ...
        >
          {this.state.titles.map((item, i) => {
            return (
              <AtTabsPane current={this.state.current} index={i} className='tabsPane'>
                <HomePage category={item} shouldRefresh={this.state.current == i}></HomePage>
              </AtTabsPane>
            )
          })}
      </AtTabs>
